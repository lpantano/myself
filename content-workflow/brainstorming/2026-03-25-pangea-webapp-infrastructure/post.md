...
![PANGEA Architecture](architecture-diagram.png)
...
React SPA (TypeScript + Tailwind)
    │  HTTPS POST /calculate-score/*
    ▼
api.pangeamodels.org (GCP VM · e2-medium)
    │
    ▼
Nginx 1.25
...
└── Reverse proxy → :8000
    │
    ▼
R Plumber API (Docker)
├── Cox PH models (survival package)
├── 20/2/20 scoring
└── PANGEA dynamic model
```
